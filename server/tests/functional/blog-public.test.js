...
describe('list', () => {
  ...
  test('a card carries exactly the public fields (no text, no ids, no e-mail, no status)', async () => {
    ...
    assert.deepEqual(Object.keys(card).sort(), ['author', 'category', 'coverUrl', 'excerpt', 'publishedAt', 'readingMinutes', 'slug', 'tags', 'title']);
...
describe('article page', () => {
  ...
  test('recommended articles: same category first, never the article itself, never a draft, at most 3', async () => {
    ...
    assert.ok(related.every((r) => Object.keys(r).sort().join() === 'author,category,coverUrl,excerpt,publishedAt,readingMinutes,slug,tags,title'));
  });
});
